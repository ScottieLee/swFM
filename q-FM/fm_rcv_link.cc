...
fm_rcv_link_sptr make_fm_rcv_link_sptr(QWidget* who, int volume_int, int hardware_decim, int rx_gain, double rx_freq, usrp_subdev_spec& spec)//FIXME ARGUMENTS!
{
    return gnuradio::get_initial_sptr(new fm_rcv_link(hardware_decim, rx_gain, rx_freq, spec, volume_int, who));//FIXME ARGUMENTS!
}

fm_rcv_link_sptr make_fm_rcv_link_sptr(int hardware_decim, int rx_gain, double rx_freq, usrp_subdev_spec& spec, int volume_int)//FIXME ARGUMENTS!
{
    return gnuradio::get_initial_sptr(new fm_rcv_link(hardware_decim, rx_gain, rx_freq, spec, volume_int));//FIXME ARGUMENTS!
}

const long   fm_rcv_link::adc_rate          = 64000000;
const int    fm_rcv_link::lpf_decim         = 30;
const float  fm_rcv_link::max_dev           = 75000;
const float  fm_rcv_link::audio_sample_rate = 96000;
const double PI                             = 3.1415927;
fm_rcv_link::fm_rcv_link(int hardware_decim,int rx_gain, double rx_freq, usrp_subdev_spec spec, int volume_int, QWidget* who):
             ...
             
{
    ...
    float mid_transition_band = 0.075;
    ...
    // volume_controller
    d_volume_controller = gr_make_multiply_const_ff(d_volume_int/30);
    std::cout << "Volume    = " << d_volume_int << std::endl;

    // reassign d_audio_sink
    d_audio_sink = audio_alsa_make_sink( static_cast<int> (audio_sample_rate));

    // qtgui sink
    d_qtsink = mqtgui_make_sink_f( 1024,
                                   gr_firdes::WIN_BLACKMAN_hARRIS,   //FFT Window
                                   0,                                // =0 since baseband freq
                                   200000,                           // rate must equal to the sample rate!!
                                   "QTfft",
                                   true,                             // Enable fft display
                                   false,                            // Endable/Disable Time domain display
                                   who);
    // poewer_indicator
    d_power_indicator = pi_make_power_indicator();

    // connect'em up
    connect(d_usrp_source_sptr, 0, d_channel_filter, 0);
    connect(d_channel_filter, 0, d_fm_demod, 0);
    connect(d_fm_demod, 0 , d_deamphasizer, 0);
    connect(d_deamphasizer, 0 , d_resampler, 0);
    connect(d_resampler, 0, d_volume_controller, 0);
    connect(d_deamphasizer, 0, d_qtsink, 0); // qtsink is after the fmdemod module, before resampler.
    connect(d_volume_controller,0, d_audio_sink, 0);
    connect(d_resampler, 0, d_power_indicator, 0);
}
