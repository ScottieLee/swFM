...
// Base class simply stores a member variable called _frequencyPrecision, and provide accessors.
...
{
  _startFrequency = 0;
  _stopFrequency = 4000;  //initial start and stop freq
  
  timespec_reset(&_lastReplot);
  resize(parent->width(), parent->height()); //!!!!!!!!!!!!!!!!!!!can we change this?

  _useCenterFrequencyFlag = false;           //!!!!!!!!!!!!!!!!!!!can we change this?
  ...
  QPalette palette;
  palette.setColor(canvas()->backgroundRole(), QColor("white"));  //set canvas background color
  ...
  _minYAxis = -150;  // this change dB range
  _maxYAxis = -90;   // this change dB range
  ...
  memset(_dataPoints, 0x0, _numPoints*sizeof(double));  //initial data set to 0;
  ...
  _zoomer = new FreqDisplayZoomer(canvas(), 0);//!!!!!!!!!!!!!!!!!!!can we change this?
#if QT_VERSION < 0x040000
  _zoomer->setMousePattern(QwtEventPattern::MouseSelect2,
			  Qt::RightButton, Qt::ControlModifier);
#else
  _zoomer->setMousePattern(QwtEventPattern::MouseSelect2,//!!!!!!!!!!!!!!!!!!!can we change this?
                          Qt::RightButton, Qt::ControlModifier);//!!!!!!!!!!!!!!!!!!!can we change this?
#endif
  _zoomer->setMousePattern(QwtEventPattern::MouseSelect3,//!!!!!!!!!!!!!!!!!!!can we change this?
                          Qt::RightButton);//!!!!!!!!!!!!!!!!!!!can we change this?

  _panner = new QwtPlotPanner(canvas());
  _panner->setAxisEnabled(QwtPlot::yRight, false);
  _panner->setMouseButton(Qt::MidButton);         //mid mouse btn for panning
  ...
  sd->setMinimumExtent( fm.width("100.00") ); //!!!!!!!!!!!!!!!!!!!can we change this?
...
void
FrequencyDisplayPlot::set_yaxis(double min, double max)   //This function is not used so far
{
...
void //This function is not used so far
...
double //This function is not used so far
				  ...
				  const double peakAmplitude, const double timeInterval)
{
  ...
  if((numDataPoints > 0) && 
    ...
    memcpy(_dataPoints, dataPoints, numDataPoints*sizeof(double)); // since _fft_plot_curve point to _dataPoints(SEE Line137),the data of curve can be changed simply via setting _dataPoints[]
    ...
    replot(); //Then all Curves got repot, but not all of'em are visible.
...
FrequencyDisplayPlot::_resetXAxisPoints()
{
  ...
  setAxisScale(QwtPlot::xBottom, _startFrequency, _stopFrequency);//!!!!!!!!!!!!!!!!Can we change this?
