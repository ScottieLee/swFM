...
fm_top_block_sptr make_fm_top_block_sptr(int hardware_decim, int rx_gain, double rx_freq, usrp_subdev_spec& spec)
{
    return gnuradio::get_initial_sptr(new fm_top_block(hardware_decim, rx_gain, rx_freq, spec));
}

fm_top_block::fm_top_block(int hardware_decim, int rx_gain, double rx_freq, usrp_subdev_spec& spec): gr_top_block("FM Receiver Core")
{
